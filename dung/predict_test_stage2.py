...
if __name__ == "__main__":
    ...
    test_df = pd.read_csv(rsna.conf.testset_stage2)
    df = pd.read_csv(rsna.conf.new_trainset)

    if args.quick:
        test_df = test_df.sample(100)

    testset = RSNADataset(df = test_df, root_dir = rsna.conf.stage2_test_dir, transform = transforms_test, datatype='test', tta = args.tta)
    ...
    for fold in args.folds:
        ...
        for epoch in args.epochs:
            ...
            tmp_df.to_csv('raw_pred/{}_yp_test_s2_fold{}_epoch{}.csv'.format(args.net, fold, epoch), index=False)
