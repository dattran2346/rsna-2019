...
from layer_wrapper import GlobalAvgPool2d
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=False, multi_grid=False, output_stride=8,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=strides[1], norm_layer=norm_layer)
        if dilated:
...
def resnext101_32x8d(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained in weakly-supervised fashion on 940 million public images 
        with 1.5K hashtags matching with 1000 ImageNet1K synsets, followed by fine-tuning on ImageNet1K dataset. 
    """
    kwargs['deep_base'] = False
    kwargs['groups'] = 32
...
def resnext101_32x16d(pretrained=False, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnext101_32x16d']))
    return model